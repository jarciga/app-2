...
Route::post('/admin/timesheet/submitEmpTimesheet', array('as' => 'submitEmpTimesheet', 'uses' => function()
{
	...
	$fromdate = date($data['schedule_date_from']);
	$todate = date($data['schedule_date_to']);
	...
	foreach($vals as $val)
	{
		...
		$timesheet->head_id = Input::get('head_ID');
		$timesheet->employee_id = $val;
		$timesheet->cutoff_starting_date = $startdate;
		$timesheet->cutoff_ending_date = $enddate;
...
Route::get('/admin/payroll', array('as' => '', 'uses' => function() {
	...
	if( !empty($groups) ) {
		...
		if ( strcmp(strtolower($groups->name), strtolower('Employee')) !== 0 ) {
			...
			return View::make('admin.payroll', ['employeeInfo' => $employeeInfo, 'getUserEmployee' => $getUserEmployee, 'getTimeSubmitted' => $getTimeSubmitted, 'getlists' => $getlists, 'getlistemps' => $getlistemps]);
...
return View::make('admin.payrolllist', ['employeeInfo' => $employeeInfo, 'getpays' => $getpays]);
...
Route::get('/admin/payroll/paylist', array('as' => '', 'uses' => function() {
	...
	//return View::make('admin.payrollsummary', ['summaries' => $summaries, 'employeeInfo' => $employeeInfo]);
return View::make('admin.paylist', ['employeeInfo' => $employeeInfo, 'getpays' => $getpays, 'getmaxs' => $getmaxs, 'employeeId' => $employeeId]);
...
Route::post('/admin/payroll/computation', array('as' => '', 'uses' => function() {
	...
	//return View::make('admin.payrollsummary', ['summaries' => $summaries, 'employeeInfo' => $employeeInfo]);
    return View::make('admin.payrollsummary', ['summaries' => $summaries, 'employeeInfo' => $employeeInfo, 'cutoffFrom' => $data["schedule_date_from"], 'cutoffTo' => $data["schedule_date_to"], 'vals' => $vals]);
...
Route::post('/admin/payroll/exportPDF', array('uses' => function()
{
		...
		->select('employees.id as empid', 'employees.employee_number as employnumber', 'lastname', 'firstname', 'basicpay', 'tax_status', 'email', 'job_title.name as jobtitle', 'departments.name as departmentname', 
				 'sumlates', 'sumabsences', 'amt_lates', 'amt_absences', 'sum_sl', 'amt_sl', 'sum_vl', 'amt_vl', 'sum_ml', 'amt_ml', 'sum_pl', 'amt_pl', 'sss_salary_loan', 'wtax', 'sss_ec', 'pagibig_ec', 'philhealth_ec', 'total_OT', 'wtax_basis', 'total_deductions', 'net_pay', 'total_nottax_earnings', 'total_tax_earnings', 'deminimis', 'ecola')
...
Route::post('/admin/payroll/saveData', array('uses' => function()
{
		...
		return View::make('admin.payrolllist', ['employeeInfo' => $employeeInfo, 'getpays' => $getpays, 'message' => $message]);
...
Route::get('/employee/report/payslip', array('as' => 'reportSummary', 'uses' => function()
{
	...
	return View::make('admin.viewpayslip', ['employeeInfo' => $employeeInfo, 'getUserEmployee' => $getUserEmployee, 'payInfos'=>$payInfos, 'payOTs'=>$payOTs]);
...
Route::get('/admin/payroll/masterfile/{empID}', array('as' => 'adminPayrollMasterfile', 'uses' => function($empID) {
	...
	if( !empty($groups) ) {
		...
		if ( strcmp(strtolower($groups->name), strtolower('Employee')) !== 0 ) {
			...
			return View::make('admin.masterfile', ['employeeInfo' => $employeeInfo, 'getUserEmployee' => $getUserEmployee, 'earnInfos' => $earnInfos, 'deductionInfos' => $deductionInfos]);        	
...
{	
	...
	if( !empty($groups) ) {
		...
		if ( strcmp(strtolower($groups->name), strtolower('Employee')) !== 0 ) {
			...
			return View::make('admin.earningsnew',['employeeInfo' => $employeeInfo]);
...
{	
	...
	if( !empty($groups) ) {
		...
		if ( strcmp(strtolower($groups->name), strtolower('Employee')) !== 0 ) {
			...
			//return View::make('admin.useredit',['employeeInfo' => $employeeInfo, 'employeeEditId' => $employeeId, 'user' => $user]);
			return View::make('admin.earningsedit', ['employeeInfo' => $employeeInfo, 'earningEditInfo' => $earningEditInfo]);				
...
Route::get('/admin/earnings/delete/{id}', array('as' => 'adminDeleteEarning', 'uses' => function($id)
{
	...
	return View::make('admin.earningsdelete', array('id' => $id, 'employeeInfo' => $employeeInfo, 'empEarningInfo' => $empEarningInfo));
...
{	
	...
	if( !empty($groups) ) {
		...
		if ( strcmp(strtolower($groups->name), strtolower('Employee')) !== 0 ) {
			...
			return View::make('admin.deductionsnew',['employeeInfo' => $employeeInfo]);
...
{	
	...
	if( !empty($groups) ) {
		...
		if ( strcmp(strtolower($groups->name), strtolower('Employee')) !== 0 ) {
			...
			//return View::make('admin.useredit',['employeeInfo' => $employeeInfo, 'employeeEditId' => $employeeId, 'user' => $user]);
			return View::make('admin.deductionsedit', ['employeeInfo' => $employeeInfo, 'deductionEditInfo' => $deductionEditInfo]);				
...
Route::get('/admin/deductions/delete/{id}', array('as' => 'adminDeleteDeduction', 'uses' => function($id)
{
	...
	//return 'Update Company';
	return View::make('admin.deductionsdelete', array('id' => $id, 'employeeInfo' => $employeeInfo, 'empdeducInfo' => $empdeducInfo));
...
Route::get('/admin/payroll/payregister', array('as' => '', 'uses' => function() {
	...
	//return View::make('admin.payrollsummary', ['summaries' => $summaries, 'employeeInfo' => $employeeInfo]);
return View::make('admin.payrollregister', ['employeeInfo' => $employeeInfo]);

}));



Route::post('/admin/payroll/payregister', array('as' => '', 'uses' => function() {
