...
class Summary extends \Eloquent {
    ...
    public function summaryJson($employeeId, $dayDateArr) {
    ...
    public function summarySearchJson($employeeSearchId, $dayDateArr) {
            
           //$employeeSearchId = Session::get('userEmployeeId');
           //$userId = Session::get('userId');
          ...
          $leaves = Leave::where('employee_id', $employeeSearchId)->get();
          ...
          foreach($leaves as $leave) {
            ...
            if ( (1 === (int) $leave->status) ) { //Aprroved
                ...
                if ( 'sick leave' === strtolower($leave->nature_of_leave) ) {
                    ...
                    DB::table('employee_summary')
                        ->where('employee_id', $employeeSearchId)
                        ->whereBetween('daydate', $leaveDateArr)
                        ->update($update);                                          
          ...
          foreach($dayDateArr as $dayDate) {                                
            ...
            $timeSheet[] = DB::table('employee_timesheet')->where('employee_id', $employeeSearchId)->where('daydate', trim($dayDate))->get();                                                            
            //$schedule[] = DB::table('employee_schedule')->where('employee_id', $employeeSearchId)->where('schedule_date', trim($dateDate->format('Y-m-d')))->get();        

            $summary[] = DB::table('employee_summary')->where('employee_id', $employeeSearchId)->where('daydate', trim($dayDate))->get();                                                                    
