...
function _workHours($timesheetTimeIn, $timesheetTimeOut, $scheduleStartTime, $scheduleEndTime, $breakTime, $hoursPerDay, $halfOfhoursPerDay, $hasBreak) {
	...
	if( timestamp($timesheetTimeIn) > timestamp($scheduleStartTime) ) {
		...
		//$tardiness = getTimeToDecimalHours($hh, $mm, $ss); //number_format($hours, 2);
		$tardiness = getTimeToDecimalHours($hh, $mm, $ss);

		$tardinessTime = number_format($tardiness, 2);
	...
	if( $hasBreak ) {						
		...
		//Halfday / Undertime set it to actual time and lates
		/*if ( $totalHours >= $halfOfhoursPerDay && $totalHours < $hoursPerDay && $tardinessTime !== FALSE ) {

			$workhours = ($totalHours - $breakTime) - $tardinessTime;
			return number_format($workhours, 2);

		} elseif ( $totalHours >= $halfOfhoursPerDay && $totalHours < $hoursPerDay && $tardinessTime === FALSE ) {			

			$workhours = $totalHours - $breakTime;
			return number_format($workhours, 2);
		
		// >= 8 Hours set it to 8.00
		} else {

			//$workhours = $totalHours;
			return 8.00; //set it to 8.00

		}*/

		if( $tardinessTime !== FALSE ) {

			//$workhours = ($totalHours - $breakTime) - $tardinessTime;
			//return $hoursPerDay;
			//return $tardinessTime;
			$workhours = $hoursPerDay - $tardinessTime;			
			return number_format($workhours, 2);	
...
function workHours($timesheetTimeIn, $timesheetTimeOut, $scheduleStartTime, $scheduleEndTime, $breakTime, $hoursPerDay, $halfOfhoursPerDay, $hasBreak) {
	...
	if( strtotime(date('Y-m-d H:i', strtotime($timesheetTimeIn))) < 
		strtotime(date('Y-m-d H:i', strtotime($scheduleStartTime))) ) {
	  
		//SET TIME IN EQUAL TO SCHEDULE START
	...
	if( strtotime(date('Y-m-d H:i', strtotime($timesheetTimeIn))) > 
		...
		$timeIn = $timesheetTimeIn;
		//$timeIn = $scheduleStartTime;

	}

	//CHECK IF TIME IN EQUAL TO START TIME 
	if( strtotime(date('Y-m-d H:i', strtotime($timesheetTimeIn))) === 
		strtotime(date('Y-m-d H:i', strtotime($scheduleStartTime))) ) {
	...
	//$totalHoursWithOvertime = totalHoursWithOvertime($timeIn, $timesheetTimeOut, $scheduleEndTime, $breakTime, $hoursPerDay, $halfOfhoursPerDay, $hasBreak);
	$totalHours = totalHours($timeIn, $timesheetTimeOut);
	...
	if( $hasBreak ) {						
		...
		//CHECK IF LESS THAN HALF DAY
		if ( $totalHours < $halfOfhoursPerDay ) {

			$timeOut = $timesheetTimeOut;
			$workhours = totalHours($timeIn, $timeOut);
			//$workhours = totalHours($timeIn, $timeOut) - $breakTime;			
			return number_format($workhours, 2);

		//CHECK IF HALF DAY OR UNDERTIME
		} elseif ( $totalHours >= $halfOfhoursPerDay && $totalHours < $hoursPerDay ) {
		...
		//OVERTIME	
		} elseif ( $overtimeHours !== FALSE ) {

			return number_format(8, 2);
...
function _totalHoursWithOvertime($timesheetTimeIn, $timesheetTimeOut, $scheduleEndTime, $breakTime, $hoursPerDay, $halfOfhoursPerDay, $hasBreak) {
...
function __totalHoursWithOvertime($timesheetTimeIn, $timesheetTimeOut, $scheduleStartTime, $scheduleEndTime, $breakTime, $hoursPerDay, $halfOfhoursPerDay, $hasBreak) {
	...
	if ( !$overtime ) {
		...
		$totalHoursWithOvertime = $workhours + $overtime;		
		return number_format($totalHoursWithOvertime, 2);
...
function totalHoursWithOvertime($timesheetTimeIn, $timesheetTimeOut, $scheduleStartTime, $scheduleEndTime, $breakTime, $hoursPerDay, $halfOfhoursPerDay, $hasBreak) {
	//CHECK IF EARLY TIME IN	
	if( strtotime(date('Y-m-d H:i', strtotime($timesheetTimeIn))) < 
		strtotime(date('Y-m-d H:i', strtotime($scheduleStartTime))) ) {
	...
	if( $hasBreak ) {						
		...
		//OVERTIME	
			...
			return number_format((8 + $breakTime + $overtimeHours), 2);
