...
class EmployeesController extends \BaseController {
		...
		if( !empty($groupName) ) :
		  ...
		  elseif( strcmp(strtolower($groupName), strtolower('Manager')) === 0 ) :                  
		    ...
		    return $employees = DB::table('employees')
		      ...
		      //->where('manager_id', $employee->id)
		      //->orWhere('supervisor_id', $employee->id)
			  //->where('employee_type', 0) //0: employee		
	...
	public function listEmployeeByGroup() {
	...
	public function showUserLists() {
		...
		$dataArr["listEmployees"] = $this->listEmployeeByGroup();
				...
				if($SentryUser) {
					...
					if( isset($userId) ) {
						...
						if ( $employee->save() ) {
							...
							DB::table('users')
							->where('id', $userId)
							->update(array('id' => $employee->id, 'employee_id' => $employee->id, 'email' => $email));
							//->update(array('employee_id' => $userId, 'email' => $email));
							...
							DB::table('users_groups')
								...
								DB::table('users')
								->where('id', $userId)
								->update(array('employee_id' => $employee->id, 'email' => $email));
							...
							for ( $i = 0; $i <= 6; $i++ ) {
								...
								DB::table('default_schedule')
									->insert(array(
										'employee_id' => $employee->id,
										'name_of_day_from' => ucwords($nameOfDay[$i]),
										'name_of_day_to' => ucwords($nameOfDay[$i]),										
										'rest_day' => $restDay,
	...
	public function showUserEdit($employeeEditId) {
		...
		$dataArr["payEdit"] = DB::table('employee_setting')->where('employee_id', $employeeEditId)->first();
		...
		return View::make('employees.index', $dataArr);		
		//return View::make('employees.index', array('dataArr' => $dataArr, 'groupName' => $dataArr['groupName']));		
	...
	public function processUserEdit($employeeEditId) {
		...
		if ( $validator->fails() ) {
		    ...
		    return Redirect::to('/admin/user/'.$employeeEditId.'/edit')->withErrors($validator)->withInput(Input::except('password', 'password_confirmation'));
			...
			try
			{
			    ...
			    if ( $userUpdate->save() ) {
					...
					// Update the employee settings for salary details
		
					$payemploy = new EmployeeSetting;
					$payempUpdate = EmployeeSetting::where('employee_id', $employeeEditId)->first();
					
					$payempUpdate->basicmonthly = $data["basicmonthly"];
					$payempUpdate->dailysal = $data["dailysal"];
					$payempUpdate->basicpay = $data["basicpay"];
					$payempUpdate->ecola = $data["ecola"];
					$payempUpdate->deminimis = $data["deminimis"];
					$payempUpdate->tax_status = $data["tax_status"];
					$payempUpdate->save();
					...
					//return Redirect::to('/admin/user/'.$employeeEditId.'/edit');
	    			return Redirect::to('/admin/user/'.$employeeEditId.'/edit')->with('message', $message);		    			
	    			//return Redirect::route('process.user.edit', array('message' => $message));
