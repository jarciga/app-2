...
//var_dump($getHeads);
//var_dump($getTimesheet);
//var_dump($getlistemps);
...
<!--div class="page-container"-->
        ...
        <!--div class="row" style="padding-bottom:20px;"-->
          ...
          <div id="content" class="col-md-12" role="main">
            ...
            <h1 class="page-header">Payroll Reminders</h1>
			
			<!-- Supervisors/Managers who submitted timesheets already -->
			<div>
				<span>Employees without timesheets (Cutoff: <?php echo $startdate . ' to ' . $enddate; ?>):</span><br>
				...
				<?php
					...
					//if(is_array($getTimeSubmitted))
					if(is_array($getlistemps))
					{
						//var_dump($getlistemps);
						foreach($getlistemps as $gEmp)
						{
							if($gEmp->employee_type == 2)
							{
								$manager = Employee::where('id', '=', $gEmp->managerid)->first();
								$headFullname = $manager->lastname.', '.$manager->firstname;
							}
							elseif($gEmp->employee_type == 0)
							{
								$manager = Employee::where('id', '=', $gEmp->supervisorid)->first();
								$headFullname = $manager->lastname.', '.$manager->firstname;
							}
							elseif($gEmp->employee_type == 1)
							{
								
								$headFullname = $gEmp->lastname.', '.$gEmp->firstname;
							}
							
							//echo $emp->id; ?>
							<tr>
								<td><?php echo $gEmp->employee_number; ?></td>
								<td><?php echo $gEmp->lastname . ', ' . $gEmp->firstname . ' ' . $gEmp->middle_name ; ?></td>
								<td><?php echo $gEmp->email; ?></td>
								<td><?php echo $gEmp->jobtitle; ?></td>
								<td><?php echo $headFullname; ?></td>
								<td><?php echo $gEmp->departmentname; ?></td>
								<td>&nbsp;</td>
							</tr>
				<?php
						}	
			...
			<!-- Supervisors/Managers who have not submitted timesheets -->
			<div>
				<span>List of Supervisors/Managers with the status of their timesheets: </span><br>
				<table class="table table-striped table-hover display table-list" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<th>Email</th>
				           		<th>Designation</th>
								<th>Department</th>
								<th>No. of Timesheets(submitted)</th>
							</tr>
						</thead>
						<tbody>
				...
				<?php
					...
					if(is_array($getlists))
					{
						...
						//var_dump($getUserEmployee);
							
						//var_dump($getlists);
							
						foreach($getlists as $gList)
						{
							//count the number of timesheets per manager/supervisor 
							...
							$num_timesheets = DB::table('employees')->get();
					
							foreach($num_timesheets as $time)
							{
									if($gList->employee_type == 1)
									{
										if($time->employee_type == 0)
										{
											if($time->supervisor_id == $gList->empid) 
											{
												$num = $num + 1;
											}
										}
										elseif($time->employee_type == 2)
										{
											if($time->manager_id == $gList->empid) 
											{
												$num = $num + 1;
											}
										}
										
										/*elseif($time->employee_type == 1)
										{
											if($time->manager_id = $gList->empid) 
											{
												$num = $num + 1;
							...
							//count the number of timesheets submitted per manager/supervisor
							$sheets = DB::table('timesheet_submitted')
							...
							foreach($sheets as $sheet)
							{
								if($gList->employee_type == 1)
								{
									if($sheet->head_id == $gList->empid)
									{
										$num_timesheet = $num_timesheet + 1;
									}
								}
								elseif($gList->employee_type == 2)
								{
									if($sheet->head_id == $gList->empid)
									{
										$num_timesheet2 = $num_timesheet2 + 1;
									}	
								}	
							}
							...
							?>
							<tr>
								<td class="check-column">{{ Form::checkbox('check[]', $gList->empid, false, array('id' => 'cb-select-' . $gList->empid, 'class' => 'checkbox')) }}</td>
								<td><?php echo $gList->employee_number; ?></td>
								<td><?php echo $gList->lastname . ', ' . $gList->firstname . ' ' . $gList->middle_name ; ?></td>
							...
							?>
								<td><?php echo $gList->departmentname; ?></td>
							<?php
								if($gList->employee_type == 1)
								{	
									echo "<td align='center'>" . $num_timesheet . " / " . $num . "</td>";
								}
								elseif($gList->employee_type == 2)
								{
									echo "<td align='center'>" . $num_timesheet2 . " / " . $num2 . "</td>";
								}
							?>
							</tr>
				<?php
						...
						<tr><td colspan="4">{{ Form::submit('Email', array('class' => '', 'class' => 'btn btn-primary')) }}</td></tr>
