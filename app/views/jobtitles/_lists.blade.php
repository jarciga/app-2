<div class="panel panel-default">
  ...
  @endif  	
	...
	{{ Form::open(array('route' => 'search.user.lists', 'id' => 'formJobtitleSearchLists', 'class' => 'form-horizontal hide hiden')) }}
...
<table class="table table-striped table-hover display list-table users" cellspacing="0" width="100%">
	...
	<tbody>
    	<?php
    	if ( $jobTitleCount !== 0 ):
			foreach($listJobTitles as $listJobTitle):	
		?>

		<tr>		           				           		
       		<td>{{ $listJobTitle->id }}</td>		           				           		
       		<td>{{ $listJobTitle->name }}</td> 
            <?php
            if( !empty($groupName) ) :
		        if ( strcmp(strtolower($groupName), strtolower('Administrator')) === 0 ||
		            strcmp(strtolower($groupName), strtolower('HR')) === 0 ) :
            ?>  
			<td><a href="{{ URL::to('/admin/jobtitle/' . $listJobTitle->id . '/edit/') }}">Edit</a></td>								           		                        	
            <?php
              endif;
            endif;
            ?>  			
