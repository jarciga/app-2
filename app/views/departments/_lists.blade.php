<div class="panel panel-default">
  ...
  @endif  	
	...
	{{ Form::open(array('route' => 'search.user.lists', 'id' => 'formDepartmentsSearchLists', 'class' => 'form-horizontal hide hiden')) }}
...
<table class="table table-striped table-hover display list-table users" cellspacing="0" width="100%">
	...
	<tbody>
		...
		<tr>		           				           		
       		<td>{{ $listDepartment->id }}</td>		           				           		
       		<td>{{ $listDepartment->name }}</td>                        		           				           
            <?php
            if( !empty($groupName) ) :
		        if ( strcmp(strtolower($groupName), strtolower('Administrator')) === 0 ||
		            strcmp(strtolower($groupName), strtolower('HR')) === 0 ) :
            ?>   			
			<td><a href="{{ URL::to('/admin/department/' . $listDepartment->id . '/edit/') }}">Edit</a></td>								           		                        	
            <?php
              endif;
            endif;
            ?>  			
    ...
    <?php echo $listDepartments->links(); ?>
